...
2. Search for prior art on the topic. Prefer the field's own venues, arXiv and
   author homepages over aggregators and blog posts.
3. Filter hard against step 1. A paper already in the vault is not a finding.
4. Report each genuinely new result as: title, authors, venue, year, DOI, and one
   sentence on why it matters *for this project specifically* — not a summary of
   its abstract.
...
finding worth knowing, and padding the list with tangential hits wastes reading time.
