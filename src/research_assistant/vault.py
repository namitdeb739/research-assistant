...
derives it from whether the body holds notes of your own, so it is a fact about
the note rather than a claim someone has to keep true by hand.
...
_GENERATED = (
    "*Generated by the `relink` command from the OpenAlex topics of the "
    "papers below "
...
def sync_read_tag(path: Path, sections: Mapping[str, str]) -> bool:
    ...
    a note's prose and quotes retracts it again. That is what keeps it a fact
